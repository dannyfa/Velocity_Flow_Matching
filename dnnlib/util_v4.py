...
class ToyDsetDynamics(Dataset):
    ...
    def __init__(self, data, dt, nForward=1, cov_dynamic_data=None, cov_static_data=None) -> None:
    ...
    def __getitem__(self, index):
        ...
        for ii in index:
            ...
            if self.cov_static_data is not None:
                # Use the left-endpoint value, and repeat it for all timepoints in this window
...
class dyn_torch_wrapper(torch.nn.Module):
    ...
    def forward(self, t, x, *args, **kwargs):
        ...
        # Compute current covariates if interpolation is enabled
        if self.cov_start is not None and self.cov_delta is not None:
            current_t = float(t)  # t is typically a scalar in NeuralODE calls
            cov_current = self.cov_start + current_t * self.cov_delta
        elif self.cov_start is not None:
            cov_current = self.cov_start
        else:
            cov_current = None
        
        # Handle ConvVNetWrapper differently
        if self.is_conv_wrapper:
            # ConvVNetWrapper expects: (xt_tau, x0_tau, cov_dynamic, cov_static, t)
            out = self.model(x, self.x0_tau, cov_current, self.cov_static, net_taus)
        else:
            # Regular model - concatenate inputs
            inputs = [x]
            if self.include_x0_tau and self.x0_tau is not None:
                inputs.append(self.x0_tau)
            if cov_current is not None:
                inputs.append(cov_current)
            if self.cov_static is not None:
                inputs.append(self.cov_static)
            
            x_concat = torch.cat(inputs, dim=-1) if len(inputs) > 1 else x
