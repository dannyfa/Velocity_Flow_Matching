...
class VFMToyNet(torch.nn.Module):
    ...
    def __init__(self, 
        ...
        if dyn_model_type in ["ToyConvUNet", "Adapted_ToyConvUNet"]:
            # For Conv architectures, use wrapper
            base_vnet = globals()[dyn_model_type](channels=channels, embed_dim=conv_embed_dim, 
                                                  img_size=img_size, img_ch=in_ch,
                                                  input_size=data_dim)
        ...
        if encoder_type in ['Latent_LargeCNN_VAE', "Adapted_Latent_LargeCNN_VAE"]:
            self.encoder = globals()[encoder_type](channels=channels, img_size=img_size, 
                                                   ...
                                                   eps=cd_eps, d_min=d_min, input_size=data_dim) 
