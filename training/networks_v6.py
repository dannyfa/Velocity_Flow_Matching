...
import torch.nn as nn
import torch.distributions as D
import torch.nn.functional as F  # For potential use, but not needed here
...
def _center_crop_or_resize(x, target_hw: int):
    # x: (B, C, H, W)
    H, W = x.shape[-2], x.shape[-1]
    if H == target_hw and W == target_hw:
        return x
    # If larger: center-crop; if smaller: bilinear resize (minimal & safe)
    if H >= target_hw and W >= target_hw:
        top = (H - target_hw) // 2
        left = (W - target_hw) // 2
...
class ToyConvUNet(torch.nn.Module):
    ...
    def __init__(self, channels=[32, 64, 128, 256], embed_dim=256, img_size=28, img_ch=1, input_size=784, cov_dim = 0):
        """
...
class Adapted_ToyConvUNet(torch.nn.Module):
    ...
    def __init__(self, channels=[32, 64, 128, 256], embed_dim=256, img_size=8, img_ch=1, input_size=2, cov_dim = 0):
        super().__init__()
        #set up basic shapes for img
        self.img_size=img_size
        self.img_ch=img_ch
...
class Latent_MLP_VAE(torch.nn.Module):
    ...
    @torch.no_grad()
    def compute_sort_perm(self, topk: int = None, mode: str = "topk"):
        d_vec = torch.exp(self.d).detach()
        D = d_vec.numel()
        if mode == "none": return torch.arange(D, device=d_vec.device)
        if mode == "full": return torch.argsort(d_vec, descending=True)
    
        # "topk"
        if topk is None: topk = getattr(self, "dims_to_keep", D)
        topk = int(max(0, min(topk, D)))
        if topk == 0: return torch.arange(D, device=d_vec.device)
        if topk == D: return torch.argsort(d_vec, descending=True)
    
        top_idx = torch.topk(d_vec, k=topk, largest=True).indices
        mask = torch.ones(D, dtype=torch.bool, device=d_vec.device)
        mask[top_idx] = False
        rest_idx = torch.arange(D, device=d_vec.device)[mask]  # preserves original order
        return torch.cat([top_idx, rest_idx], dim=0)
    
    @torch.no_grad()
    def apply_perm_inplace(self, perm: torch.Tensor):
        """Reorder parameters to match 'perm' on the output dimension (μ index)."""
        # μ head: [out=d, in=feat]
        self.mu.weight.copy_(self.mu.weight[perm, :])
        if self.mu.bias is not None:
            self.mu.bias.copy_(self.mu.bias[perm])
    
        # per-dimension parameters/buffers
        self.U.copy_(self.U[perm, :])       # [d, r]
        self.d.copy_(self.d[perm])          # [d]
    
        # keep buffers aligned too (safe even if values are uniform)
        if hasattr(self, 'd_max_diag'):
            self.d_max_diag.copy_(self.d_max_diag[:, perm])
        if hasattr(self, 'd_min_diag'):
            self.d_min_diag.copy_(self.d_min_diag[:, perm])
    
    def __init__(self, input_size, output_size, dims_to_keep, num_hidden, hidden_size=10, eps=1e-5,
                 d_min=1e-10, rank=None):

        super().__init__()
        self.dims_to_keep = dims_to_keep
        
        d = output_size
        r = dims_to_keep*2 if (rank is None) else min(rank, output_size)
        ...
        keep_cap = 1e20 # let's never upper bound the dims_to_keep
    ...
    def encode(self, x):
        ...
        d_vec = torch.clamp(d_vec,
                            min=self.d_min_diag.to(x.device).squeeze(0),
                            max=self.d_max_diag.to(x.device).squeeze(0))  # [d]

        # # permutation
        # k = int(self.dims_to_keep)
        # head_perm = torch.argsort(d_vec[:k], descending=True)
        # tail_perm = torch.arange(k, d_vec.numel(), device=d_vec.device) 
        # perm = torch.cat([head_perm, tail_perm], dim=0)
        # mu = mu[:, perm]            # columns of μ follow the new latent order
        # d_perm = d_vec[perm]        # [d]
        # Up = self.U[perm, :]      # permute rows (latent dims)

        # no permutation
        perm = torch.arange(d_vec.numel(), device=d_vec.device)
        mu = mu  # unchanged
        d_perm = d_vec  # unchanged
        Up = self.U     # unchanged

        L_raw = torch.tril(Up @ Up.T)

        # # option 1: just lower triagular
        # L = L_raw + 1e-6 * torch.eye(L_raw.size(0), device=x.device, dtype=x.dtype)

        # option 2: further unit diagonal
        L = L_raw - torch.diag_embed(torch.diag(L_raw))         # zero out diag
        L = L + torch.eye(L.size(0), device=x.device, dtype=x.dtype)  # unit diag
    ...
    def rsample(self, x):
        ...
        # z = mu + eps 
        z = torch.einsum('ij, bjk -> bik', L_tril, mu.unsqueeze(-1)).squeeze(-1)
...
class Latent_CNN_VAE(torch.nn.Module):
    ...
    def __init__(self, img_resolution=28, img_ch=1, dims_to_keep=784, eps=1.0, d_min=1e-10, rank = None):
        ...
        d = self.input_dim
        r = dims_to_keep * 2 if (rank is None) else min(rank, d)
        ...
        if dims_to_keep < self.input_dim: 
            ...
            self.d_max_diag = torch.ones(self.input_dim).unsqueeze(0) * keep_cap #1, dim 
        ...
        # For mean encoder
        self.conv1 = torch.nn.Conv2d(img_ch, 16, kernel_size=5, stride=2)
        self.conv2 = torch.nn.Conv2d(16, 32, kernel_size=5, stride=2)
        with torch.no_grad():
            dummy = torch.zeros(1, img_ch, img_resolution, img_resolution)
    ...
    def encode(self,x):
        ...
        L_raw = torch.tril(Up @ Up.T)                                   # [d, d]
        L = L_raw - torch.diag_embed(torch.diag(L_raw))                 # zero diag
        L = L + torch.eye(L.size(0), device=x.device, dtype=x.dtype)
...
class Latent_LargeCNN_VAE(torch.nn.Module):
    ...
    def __init__(self, channels=[32, 64, 128, 256], img_size=28, img_ch=1,
               dims_to_keep=784, eps=1.0, d_min=1e-15, input_size=784, rank=None):
        ...
        with torch.no_grad():
            dummy = torch.zeros(1, img_ch, img_size, img_size)
    ...
    def encode(self, x): 
        ...
        # let's turn off the permutation
        perm = torch.arange(d_vec.numel(), device=d_vec.device)
        mu = mu  # unchanged
        d_perm = d_vec  # unchanged
        Up = self.U     # unchanged
...
class Adapted_Latent_LargeCNN_VAE(torch.nn.Module):
    ...
    def __init__(self, channels=[32, 64, 128, 256], img_size=8, img_ch=1, input_size=2, dims_to_keep=2, eps=1.0, d_min=1e-15, rank = None):
        super().__init__()
        #set up basic shapes for img
        self.img_size=img_size
        self.img_ch=img_ch
        ...
        if dims_to_keep < self.input_size: 
            ...
            self.d_max_diag = torch.ones(self.input_size).unsqueeze(0) * keep_cap #1, dim 
    ...
    def rsample(self, x):
        mu, D, L = self.encode(x)
        d_sqrt = torch.sqrt(torch.diagonal(D))   # [d]
        L_scaled = L * d_sqrt                    # column-wise scale -> [d,d]
        z = mu @ L_scaled.T  
        return z
...
class ConvVNetWrapper(nn.Module):
    ...
    def __init__(self, base_model, img_ch, img_size, data_dim,
                 include_x0_tau, dim_cov_dynamic, dim_cov_static, use_t_dyn=True):
        super().__init__()
        self.cov_norm = None
        ...
        self.include_x0_tau = bool(include_x0_tau)
        ...
        if self.is_image_data:
            ...
            lag_elems    = int(self.dim_cov_static) - self.static_elems      # multiple of C*H*W
            self.lag_ch  = lag_elems // (self.img_size * self.img_size) # (k+1)*C
            assert (self.dim_cov_static - self.static_elems) % self.pixels == 0, "dim_cov_static must equal static_elems + (K+1)*C*H*W"
            
            self.lag_tempmix = None
            if self.lag_ch > 0:
                C = self.img_ch
                L = self.lag_ch // C          # (K+1) lags per channel
                assert self.lag_ch % C == 0 and L >= 1
                self.L_lag = L
                self.lag_tempmix = nn.Conv1d(in_channels=C, out_channels=C,
                                             kernel_size=3, padding=1, groups=C, bias=True)

                with torch.no_grad():
                    self.lag_tempmix.weight.zero_()         # shape (C, 1, 3)
                    self.lag_tempmix.bias.zero_()
                    self.lag_tempmix.weight[:, 0, 1] = 1.0  # center tap

            # how many dims will go into cov_embed (dynamic + true static only)
            self.vec_cov_dim = (dim_cov_dynamic or 0) + self.static_elems
            ...
            if (self.dim_cov_dynamic + self.dim_cov_static) > 0 or self.use_t_dyn:
                in_dim = (self.dim_cov_dynamic or 0) + (self.dim_cov_static or 0) + (1 if self.use_t_dyn else 0)
                self.cov_to_bias = nn.Linear(in_dim, self.img_ch)
        
            # One 1×1 conv to fuse [xt_img, (x0_img)?, lag_maps] → C channels
            in_ch = self.img_ch + (self.img_ch if self.include_x0_tau else 0) + self.lag_ch
            self.in_proj = nn.Conv2d(in_ch, self.img_ch, kernel_size=1, bias=True)


        else:
            # Non-image: concat vectors then project back to data_dim
            total_input_dim = data_dim + (data_dim if include_x0_tau else 0)
            self.linear_projection = nn.Linear(total_input_dim, data_dim)

    def forward(self, xt_tau, x0_tau, cov_dynamic, cov_static, t, t_dyn=None):
        """
        xt_tau: (B, data_dim)
        ...
        if self.is_image_data:
            ...
            if self.include_x0_tau:
                if x0_tau is not None:
                    pieces.append(x0_tau.view(B, self.img_ch, H, W))
                else:
                    pieces.append(torch.zeros_like(xt_img))


            cov_vec_full = None
            if (cov_dynamic is not None and self.dim_cov_dynamic > 0) or (cov_static is not None and self.dim_cov_static > 0):
                ...
                cov_vec_full = torch.cat(parts, dim=-1) if parts else None
            else:
                cov_vec_full = (t_dyn.view(-1, 1) if (self.use_t_dyn and (t_dyn is not None)) else None)

            lag_maps = None
            if (cov_static is not None) and (self.dim_cov_static > 0) and (self.lag_ch > 0):
                lag_elems = self.dim_cov_static - self.static_elems
                if lag_elems > 0:
                    lag_flat = cov_static[..., self.static_elems:self.static_elems + lag_elems]
                    lag_maps = lag_flat.view(B, self.lag_ch, H, W)
            
            if (lag_maps is not None) and (self.lag_tempmix is not None):
                C, L = self.img_ch, self.L_lag
                lag5 = lag_maps.view(B, C, L, H, W)
                x = lag5.permute(0,3,4,1,2).contiguous().view(B*H*W, C, L)
                x = self.lag_tempmix(x)
                x = x.view(B, H, W, C, L).permute(0,3,4,1,2).contiguous()
                lag_maps = x.view(B, C*L, H, W)
            
            # zero-fill if in_proj expects lag channels but none present this batch
            ...
            if (self.cov_to_bias is not None) and (cov_vec_full is not None):
                cov_bias = self.cov_to_bias(cov_vec_full).view(B, self.img_ch, 1, 1)
                img_feat = img_feat + cov_bias

            
            # Pass to base model; try forwarding cov_vec
            try:
                y = self.base_model(img_feat, t, None)
            except TypeError:
                y = self.base_model(img_feat, t)

            if y.dim() == 2 and y.shape[1] == self.img_ch * self.img_size * self.img_size:
                y = y.view(B, self.img_ch, self.img_size, self.img_size)
            elif y.dim() != 4:
                raise RuntimeError(f"Expected image tensor or flat (B,{self.img_ch*self.img_size*self.img_size}), got {tuple(y.shape)}")

            
            # Enforce exact spatial size (handles 63/65 off-by-1s from transposed convs)
        ...
        if self.include_x0_tau and (x0_tau is not None):
            x_parts.append(x0_tau)
        combined_x = torch.cat(x_parts, dim=-1) if len(x_parts) > 1 else xt_tau

        projected = self.linear_projection(combined_x)  # (B, data_dim)

        cov_vec = (t_dyn.view(-1, 1) if (self.use_t_dyn and (t_dyn is not None)) else None)
        if self.total_cov_dim > 0 and (cov_dynamic is not None or cov_static is not None):
            parts = []
            if (self.dim_cov_dynamic or 0) > 0 and cov_dynamic is not None: parts.append(cov_dynamic)
            if (self.dim_cov_static  or 0) > 0 and cov_static  is not None: parts.append(cov_static)
            if parts:
                base_cov = torch.cat(parts, dim=-1)
                if self.cov_norm is not None:
                    base_cov = self.cov_norm(base_cov)
                cov_vec = base_cov if cov_vec is None else torch.cat([base_cov, cov_vec], dim=-1)


        try:
            return self.base_model(projected, t, cov_vec)  # Adapted_ToyConvUNet accepts c
        except TypeError:
...
class VFMToyNet(torch.nn.Module):
    ...
    def __init__(self, 
                 ...
                 d_min=1e-15,
                 save_dir='',
                 include_x0_tau=True,  # Whether to include x0_tau in dynamics net
        ...
        if include_x0_tau:
            vnet_input_dim += data_dim  # x0_tau
        ...
        if dyn_model_type in ["ToyConvUNet", "Adapted_ToyConvUNet"]:
            total_cov_dim = (dim_cov_dynamic or 0) + (dim_cov_static or 0)
        ...
        if encoder_type in ['Latent_LargeCNN_VAE', "Adapted_Latent_LargeCNN_VAE"]:
            ...
            self.encoder = globals()[encoder_type](input_size=data_dim, output_size=data_dim,
                                                   ...
                                                   hidden_size=width_encoder, eps=cd_eps, d_min=d_min, rank=encoder_rank) 
        else:
            self.encoder = globals()[encoder_type](img_resolution=img_size, img_ch=in_ch, 
                                                   dims_to_keep=dims_to_keep, eps=cd_eps,
    ...
    def forward(self, x0_1, xdt_1, taus, ts, dt, tau_flowmatcher, t_flowmatcher, 
        ...
        if cov_dynamic_0 is not None and cov_dynamic_dt is not None and self.dim_cov_dynamic > 0:
            t_interp = (ts/dt).clamp(0, 1).unsqueeze(-1)
