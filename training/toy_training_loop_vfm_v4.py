...
#     np.savez(os.path.join(run_dir, 'dataset_samples.npz'), samples=dset_samples)
    ...
    net = dnnlib.util_v4.construct_class_by_name(**network_kwargs) # subclass of torch.nn.Module
    ...
    loss_fn = dnnlib.util_v4.construct_class_by_name(**loss_kwargs) 
    optimizer = dnnlib.util_v4.construct_class_by_name(params=net.parameters(), **optimizer_kwargs) # subclass of torch.optim.Optimizer
    ...
    while True:
        ...
        if (state_dump_ticks is not None) and (done or cur_tick % state_dump_ticks == 0) and dist.get_rank() == 0:
            ...
            if dataset_kwargs.balls_dset_specs != None:
                ...
                orig_traj, enc_traj = dnnlib.util_v4.sim_encoded_trajs(gt_trajs, net.encoder, device)
                orig_fig, enc_fig = dnnlib.util_v4.plot_encoded_trajs(orig_traj, enc_traj, \
