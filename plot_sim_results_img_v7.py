...
class _FlowFlatAdapter(torch.nn.Module):
    ...
    def forward(self, x, t):
        ...
        if y.dim() == 4:
            y = _crop_or_resize4d(y, self.H, self.W).reshape(B, -1)  # -> (B, D)
        else:
            y = y.reshape(B, -1)
        return y
...
def plot_frame(data, T, trial_idx, mean_img=None, idxs=None, n=6, colorbar=True, figsize=(12,2)):
    if idxs is None:
        idxs = np.linspace(0, T-2, n, dtype=int)

    X = data[trial_idx][:, 0]                       # [T, H, W]
    base = (X + mean_img) if mean_img is not None else X
    vmin, vmax = float(base.min()), float(base.max())

    fig, axes = plt.subplots(1, len(idxs), figsize=figsize)
    last_im = None
    for ax, i in zip(axes, idxs):
        img = (data[trial_idx][i, 0] + mean_img) if mean_img is not None else data[trial_idx][i, 0]
        last_im = ax.imshow(img, cmap='gray', vmin=vmin, vmax=vmax)
        ax.set_title(f"t={i}")
        ax.axis("off")
    if colorbar:
        fig.colorbar(last_im, ax=axes.ravel().tolist(), orientation='horizontal', fraction=0.05, pad=0.10)
    plt.tight_layout()
...
#     include_x0_tau=False, oracle=False,
...
#         """Oracle: lag from truncated frames [t-k..t], pad on left; map to τ if τ<1."""
#         lag_tensors = []
#         for j in range(B):
...
#             # ((k+1), C, H, W) -> ((k+1), D)
#             fr = torch.as_tensor(np.stack(frames, 0), dtype=torch.float32, device=device)
#             fr_flat = fr.view(fr.shape[0], -1)
#             if tau < 1.0:
#                 assert flow_for_ode is not None, "flow_net required for oracle τ<1.0."
#                 with torch.no_grad():
#                     fr_flat = util.calc_flow_trajectories(flow_for_ode, fr_flat, 1.0, tau, nt=2)[-1]
#             lag_tensors.append(fr_flat.view(-1))  # ((k+1)*D)
#         return torch.stack(lag_tensors, 0)  # (B,(k+1)*D)

#     # --- init τ-history lag from current τ state ---
#     x0_tau_img = curr_tau_pts.squeeze(1).view(B, C, H, W)       # (B,C,H,W)
#     lag_vec = x0_tau_img.repeat(1, lag_k + 1, 1, 1).view(B, -1) if lag_k > 0 else curr_tau_pts.squeeze(1)
...
#         if oracle:
#             # x0 from data -> map to τ if needed
#             x0_tmp = torch.stack([
...
#                 assert flow_for_ode is not None, "flow_net required for oracle τ<1.0."
#                 with torch.no_grad():
#                     x0_tmp = util.calc_flow_trajectories(flow_for_ode, x0_tmp, 1.0, tau, nt=2)[-1]
#             lag_now = build_lag_from_truncated(ii)  # already τ-mapped when τ<1
#         else:
...
#         # roll tau-history
#         if (not oracle):
#             ximg_tau = x_next.view(B, C, H, W)
#             if lag_k > 0:
#                 if ii == 0 and 'lag_img_stack' not in locals():
#                     lag_img_stack = ximg_tau.repeat(1, lag_k + 1, 1, 1)
#                 else:
#                     lag_img_stack = torch.cat([lag_img_stack[:, C:], ximg_tau], dim=1)
...
def generate_traj_image_onthefly(
    ...
    if use_full:
        ...
        for j in range(B):
            ...
            off = Tf - Tt
            # Expect off == lag_k in your setup, but do not hard fail
            offsets.append(off)
    else:
        offsets = [0] * B
    ...
    # --- x(τ) at first frame (from truncated dataset) ---
    x0 = np.stack([np.asarray(dset_samples[i][0]).reshape(-1) for i in range(B)], 0)
    curr_tau_pts = torch.as_tensor(x0, dtype=torch.float32, device=device).unsqueeze(1)  # (B,1,D)
    ...
    def _tau_map_frames(fr_flat):
        # fr_flat: (N, D)
        if tau < 1.0:
            assert flow_for_ode is not None, "flow_net required when tau<1.0."
            with torch.no_grad():
                fr_flat = util.calc_flow_trajectories(flow_for_ode, fr_flat, 1.0, tau, nt=2)[-1]
        return fr_flat

    def build_lag_from_truncated(ii):
        """Pad on left using first truncated frame."""
        lag_tensors = []
        for j in range(B):
            X = np.asarray(dset_samples[j])  # (Tj, C, H, W)
            Tj = X.shape[0]
            ...
            if lag_k == 0:
                ...
                start = max(0, t - lag_k)
                frames = [X[tt] for tt in range(start, t + 1)]
                if len(frames) < (lag_k + 1):
                    frames = [X[0]] * ((lag_k + 1) - len(frames)) + frames

            fr = torch.as_tensor(np.stack(frames, 0), dtype=torch.float32, device=device)  # (L,C,H,W)
            fr_flat = fr.view(fr.shape[0], -1)  # (L,D)
            fr_flat = _tau_map_frames(fr_flat)
            lag_tensors.append(fr_flat.view(-1))  # ((k+1)*D)
        return torch.stack(lag_tensors, 0)  # (B,(k+1)*D)

    def build_lag_from_full(ii):
        """True history from full dataset aligned by offset = T_full - T_trunc."""
        lag_tensors = []
        for j in range(B):
            Xf = np.asarray(dset_samples_full[j])  # (Tf, C, H, W)
            Tf = Xf.shape[0]
            t_abs = min(ii + offsets[j], Tf - 1)  # absolute time in full

            if lag_k == 0:
                frames = [Xf[t_abs]]
            else:
                frames = []
                for tt in range(t_abs - lag_k, t_abs + 1):
                    if tt < 0:
                        frames.append(Xf[0])
                    else:
    ...
    def init_lag_from_full_at_start():
        # history for the initial truncated frame corresponds to ii=0 in truncated
        # which is t_abs = offsets[j] in full
        lag0 = build_lag_from_full(ii=0)  # (B, (k+1)*D)
        if lag_k > 0:
            # also keep image-stacked form for efficient rolling
            hist = lag0.view(B, lag_k + 1, D).view(B, lag_k + 1, C, H, W)
            lag_img_stack = hist.view(B, (lag_k + 1) * C, H, W)  # (B,(k+1)*C,H,W)
            lag_vec = lag_img_stack.view(B, -1)
            return lag_img_stack, lag_vec
        else:
            return None, lag0  # lag_vec is (B,D)

    # --- init τ-history lag ---
    if lag_k > 0:
        if use_full:
            lag_img_stack, lag_vec = init_lag_from_full_at_start()
        else:
            # fallback: pad by repeating current τ state
            x0_tau_img = curr_tau_pts.squeeze(1).view(B, C, H, W)
            lag_img_stack = x0_tau_img.repeat(1, lag_k + 1, 1, 1)  # (B,(k+1)*C,H,W)
            lag_vec = lag_img_stack.view(B, -1)
    else:
        lag_img_stack = None
        lag_vec = curr_tau_pts.squeeze(1)  # (B,D)
    ...
    for ii in tqdm(range(n_step - 1)):
        # state & lag for this step
        if oracle:
            # x0 from truncated data -> map to τ if needed
            x0_tmp = torch.stack([
                torch.as_tensor(dset_samples[j][min(ii, len(dset_samples[j]) - 1)],
                                dtype=torch.float32, device=device).reshape(-1)
                for j in range(B)
            ...
            if lag_k > 0:
                lag_now = build_lag_from_full(ii) if use_full else build_lag_from_truncated(ii)
            else:
                lag_now = x0_tmp  # (B,D)
        else:
            x0_tmp = curr_tau_pts.squeeze(1)  # (B,D)
            lag_now = lag_vec                 # (B,(k+1)*D) or (B,D)

        # clamp lag
        if clamp_range is not None:
            if cmin is not None and lag_k > 0:
                lag_now = torch.clamp(lag_now.view(B, lag_k + 1, C, H, W), min=cmin, max=cmax).view(B, -1)
            elif gmin is not None:
                lag_now = lag_now.clamp(min=gmin, max=gmax)
        ...
        # roll τ-history (only in non-oracle mode)
        if not oracle:
            if lag_k > 0:
                ximg_tau = x_next.view(B, C, H, W)
                lag_img_stack = torch.cat([lag_img_stack[:, C:], ximg_tau], dim=1)  # shift, append
                lag_vec = lag_img_stack.view(B, -1)
            else:
                lag_vec = x_next

        # record lag actually used at this step (do not record the rolled version)
        lag_cov_list.append(lag_now)
